...
public class Paging {
	...
	public Paging(String tbName, String bkNum, String pmCol, String pmKey) {
		...
		/*********************************
		 * 15. 페이징 링크 생성하기 ______________________________
		 * 
		 * 1)시작 레코드번호 : startNum 2)페이지당 레코드개수 : onePageCnt 3)전체 레코드수 : totalCnt 4)리스트
		 * 그룹수 : listGroup (전체개수 ÷ 페이지당개수) 5)남은 레코드수 : etcRecord 6)페이징링크 코드 저장변수 pgCode
		 * 
		 *********************************/
		try {
			// 15-1. 전체 레코드 수 구하기
			...
			// 만약 검색어가 있으면 쿼리 추가하기!
			if (pmKey != null) {
				cntQuery += " WHERE `" + pmCol + "` " + "LIKE \"%" + pmKey + "%\" ";
			...
			pgdto.setLimit(pgdto.getEtcRecord() == 0 ? pgdto.getListGroup() : pgdto.getListGroup() + 1);
			// 나머지가 있으면 1페이지 더 추가!

			// ####### 페이징 블록 셋팅하기 ########
			// 8.페이징 단위개수(oneBlockCnt)
			// 9.페이징 그룹수 : (리스트그룹수+남은레코드수) ÷ 페이징 단위개수 (blockGroup)
			pgdto.setBlockGroup((pgdto.getListGroup() + pgdto.getEtcRecord()) / pgdto.getOneBlockCnt());
			// 10.남은 페이징수 : (리스트그룹수+남은레코드수) % 페이징 단위개수 (etcBlock)
			pgdto.setEtcBlock((pgdto.getListGroup() + pgdto.getEtcRecord()) % pgdto.getOneBlockCnt());
